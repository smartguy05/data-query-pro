...
├── databases.json           # Actual connection config (gitignored)
├── reports.json.example     # Example shared-reports config
└── reports.json             # Actual shared reports (gitignored, optional)
```
...
- **Shared Reports**: Reports can be deployed via `config/reports.json` (mirrors `databases.json`)
  - Loaded via `getServerReports()` (`lib/server-config.ts`) + `/api/config/reports`, merged fresh on every load by `LocalStorageProvider` (never written to localStorage), marked `source: "server"`
  - Always-present & read-only in UI ("Server Config" badge, no edit/delete/favorite); Clone/Copy-to-Connection make local copies
  - Each report's `connectionId` must match a connection `id` in `databases.json`
  - Generated by the Reports page "Export Reports" dialog (`components/export-reports-dialog.tsx`); non-auth-mode only
- Each connection has:
...
- The UI library is synced to **Claude Design** as **DataQuery Pro UI** so the design agent builds new screens with the project's *real* components. Project: https://claude.ai/design/p/91e99f55-b714-4087-99d2-0a07ebf0a369
- **Sync inputs live in `.design-sync/`** (committed): `config.json` (package shape, synth-entry, carded allowlist), `conventions.md` (the design-agent styling guide — keep it true), `previews/*.tsx` (54 authored preview compositions), `overrides/source-kit.mjs` (converter fork), `NOTES.md` (gotchas), `tw.preview.cjs` (Tailwind compile config).
- **~54 components are carded; ~265 exports stay importable** on `window.DataQueryProUI`. App-chrome coupled to `next-auth`/`next/navigation` is intentionally excluded (it can't run in the design runtime).
- **To re-sync** after component changes, re-run the **`/design-sync`** skill — it reuses the committed `.design-sync/` inputs and only re-verifies what changed. To card a new component, add `"<ExportName>": true` to `componentSrcMap` and re-sync.
- Full guide: [docs/guides/design-system.md](./docs/guides/design-system.md)
