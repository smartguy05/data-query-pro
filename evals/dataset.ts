...
// CORE set (default run, 16 questions): all 8 phase4-tagged questions plus at
// least one of every bucket/mode — filters, row-count, multi-join,
// scalar+unordered aggregation, NULL-skipping, volatile date math, ordered
// top-N, derived-view values, group-by join, NULL filter, 2 ambiguous.
...
// EXTENDED set (16 questions): verified goldens excluded from the default run
// to halve API cost; enabled via --extended.
export const EXTENDED_QUESTIONS: EvalQuestion[] = [
  // ── Simple counts ─────────────────────────────────────────────────────────
  {
    id: "Q02",
